...
public class CompletableFutureContoller {
	...
	public CompletableFuture<ModelAndView> indexAsync() {
		...
		return someService.getMessage().thenApply(msg -> new ModelAndView("message", "message", msg));
	}

	@RequestMapping(value = "/async2", method = RequestMethod.GET)
	public ModelAndView indexAsync2() {
		LOG.info("Servlet Thread Id = '" + Thread.currentThread().getName() + "'.");
		return new ModelAndView("message", "message", someService.getMessage2());
	}
